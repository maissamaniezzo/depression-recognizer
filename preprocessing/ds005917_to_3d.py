...
Exemplo:
  python ./preprocessing/ds005917_to_3d.py \
      --in_dir ./ds005917 --out_dir ./dataset_3d \
      --dataset_name ds005917 \
...
def find_bold_rest_baseline(in_dir: Path) -> List[Path]:
    """
    Procura repouso baseline em: **/ses-b0/func/*task-rest_bold.nii.gz
    Se não achar nada, cai para **/ses-b0/func/*_bold.nii.gz.
    """
    pats = list(in_dir.glob("**/ses-b0/func/*task-rest_bold.nii.gz"))
    if not pats:
        pats = list(in_dir.glob("**/ses-b0/func/*_bold.nii.gz"))
...
def make_out_path(in_file: Path, in_root: Path, out_root: Path,
    ...
    Ex.: OUT/ds005917/sub-XX/ses-b0/func/<stem>_3d.nii.gz
    """
...
def main():
    ...
    files = find_bold_rest_baseline(in_root)

    if not files:
        print("Nenhum run de repouso encontrado (ses-b0/func/*task-rest_bold.nii.gz ou *_bold.nii.gz).")
        return
